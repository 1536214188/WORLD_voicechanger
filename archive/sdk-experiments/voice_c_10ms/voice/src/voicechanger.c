...
static void process_world(double* input, double* output)
{
    ...
    /* Hann window 减少 block edge noise */
...
DWORD WINAPI world_thread(LPVOID arg)
{
    while (g_vc.running)
    {
        ...
        if (g_vc.input_size >= VC_STEP_SAMPLES)
        {
            ...
            memcpy(block, g_vc.prev_overlap,
            ...
            double output[VC_ANALYSIS_SAMPLES];

            process_world(block, output);
...
int vc_init()
{
    ...
    for (int i = 0; i < VC_ANALYSIS_SAMPLES; i++)
        hann_window[i] =
        0.5 - 0.5 * cos(2 * M_PI * i / (VC_ANALYSIS_SAMPLES - 1));
    ...
    printf("VoiceChanger initialized\n");
...
void vc_set_params(double pitch, double formant)
{
    ...
    printf("Params pitch=%.2f formant=%.2f\n", pitch, formant);
}

PCMFrame20ms vc_process(PCMFrame20ms in)
{
    PCMFrame20ms out;
    ...
    /* 防止 FIFO overflow */
    if (g_vc.input_size + VC_FRAME_SAMPLES < VC_ANALYSIS_SAMPLES * 3)
    {
        for (int i = 0; i < VC_FRAME_SAMPLES; i++)
            g_vc.input_fifo[g_vc.input_size++] =
            in.data[i] / 32768.0;
    }
