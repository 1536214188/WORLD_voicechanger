...
PCMFrame10ms vc_process(PCMFrame10ms in)
{
    ...
    {
        PcmSummary input_summary = summarize_pcm(in.data, vc->frame_samples);
        PcmSummary output_summary = summarize_pcm(out.data, vc->frame_samples);
        double diff_avg = 0.0;
        int same_count = 0;

        summarize_pcm_diff(in.data, out.data, vc->frame_samples,
            &diff_avg, &same_count);
        ...
        VC_LOG("vc_process #%d input first=%d mid=%d last=%d min=%d max=%d input_rms=%.2f output first=%d mid=%d last=%d min=%d max=%d output_rms=%.2f diff_avg=%.2f same_count=%d/%d",
            ...
            diff_avg, same_count, vc->frame_samples);
    }
