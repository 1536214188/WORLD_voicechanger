...
struct AudioContext {
    ...
    std::atomic<int> frame_mismatch_count;
};
    ...
    void* userData)
{
    ...
    if (framesPerBuffer != (unsigned long)ctx->frame_samples) {
        int mismatch_index = ++ctx->frame_mismatch_count;
        if (mismatch_index == 1 || (mismatch_index % 50) == 0) {
            std::cerr << "[Project4] framesPerBuffer mismatch actual="
                      << framesPerBuffer << " expected="
                      << ctx->frame_samples << "\n";
        }
    ...
    std::memcpy(input_frame.data, in,
        (size_t)ctx->frame_samples * sizeof(int16_t));
    ...
    if (callback_index == 1 || (callback_index % 50) == 0) {
        std::cout << "[Project4] callback #" << callback_index
...
int main()
{
    ...
    std::cout << "VoiceChangerProject4 PortAudio dynamic sampling test\n";
    ...
    frame_samples = samples_from_ms(sample_rate, VC_FRAME_MS);
    ...
    context.frame_mismatch_count = 0;
    ...
    std::cout << "Stopped. callbacks=" << context.callback_count
              << " frame_mismatches=" << context.frame_mismatch_count
              << "\n";
    return 0;
}
